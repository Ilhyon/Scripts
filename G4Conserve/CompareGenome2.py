...
``ConmpareGenome2`` **module description**:
...
def CreationIndex(Index,headerTest,headerIndex):
	...
	return Index
###########################################################################################################################################
def conditionInclusifGene (geneExt,start, end, startBorder, endBorder):
...
def main () :
	...
	print 'Index chr done'

	## Get OQ in gene
	OQInGene=[]
	for key, listeOQ in IndexOQ.items():
		chrm=key.split("|")[0]
		strand=key.split("|")[1]
		for OQ in listeOQ:
			inGene=False
			startOQ=OQ.split('-')[0]
			endOQ=OQ.split('-')[1]
			listeGene=sorted(IndexGenome.get(key)) # recup gene in this chr and strand
			i=0
			while( i <len(listeGene)  and inGene == False):
				startGene=listeGene[i].split('-')[0]
				endGene=listeGene[i].split('-')[1]
				inGene=conditionInclusifGene (geneExt,startOQ, endOQ, startGene, endGene) # condition of OQ in gene or not
				if (inGene == True):
					if (OQ not in OQInGene):
						OQInGene.append(chrm+':'+OQ+'|'+strand)

				i+=1

	print 'Get OQ in gene done'


	outfilename= open('/home/local/USHERBROOKE/bels2814/Documents/These/ChambersArticle/moreData/result/OQInGene_'+condition+'.txt',"w")	# file opening for reading
	for element in OQInGene:
		outfilename.write(element+'\n')	
	print 'extract OQ in gene done'	
	
	# Get OQ common
	OQCommon=[]
	for key, listeOQ in IndexOQ.items(): # for each chr and strand
		chrm=key.split("|")[0]
		strand=key.split("|")[1]
		for OQ in listeOQ: 	# for each OQ
			if ((chrm+':'+OQ+'|'+strand) in OQInGene):# if OQ in gene
				commonOQ=False
				startOQ=OQ.split('-')[0]
				endOQ=OQ.split('-')[1]	
				listeG4=sorted(IndexG4.get(key)) # recup G4 in this chr and strand
				i=0
				while( i <len(listeG4)  and commonOQ == False):
					startG4=int(listeG4[i].split('-')[0])-G4Ext
					endG4=int(listeG4[i].split('-')[1])+G4Ext
					commonOQ= conditionOverlap(startOQ, endOQ, startG4, endG4) # condition of OQ overlapp G4r
					if (commonOQ == True):
						if (OQ not in OQCommon):
							OQCommon.append(chrm+':'+OQ+'|'+strand)
					i+=1
			else:
				continue


	print 'Get OQ common done'

	outfilename= open('/home/local/USHERBROOKE/bels2814/Documents/These/ChambersArticle/moreData/result/OQCommon_'+condition+'.txt',"w")	# file opening for reading
	for element in OQCommon:
		outfilename.write(element+'\n')	
	print 'extract OQ common done'
	...
	print 'Get G4 common done'

	outfilename= open('/home/local/USHERBROOKE/bels2814/Documents/These/ChambersArticle/moreData/result/G4Common_'+condition+'.txt',"w")	# file opening for reading
