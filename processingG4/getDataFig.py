...
def getFigBysubClass(df, path, nameClass):
	...
	tmp = pd.DataFrame()
	tmp = tmp.append(df)
	dicoNbTrClass = countTranscript.getFig3Percent(path, sp)
	dicoNbTrBt = countTranscript.getFig5Percent(path, sp)
	del tmp['nuclA']
	del tmp['nuclT']
	del tmp['nuclN']
	del tmp['Type']
...
def getFigDensityByLocation(df, path, nameClass):
	...
	"""
	tmp =  pd.DataFrame()
	dicoTotMulti = countLocation.getAllLocNumMulti(path, sp, 'Class')
	dicoWt = countLocation.getpG4WtLocNum(path, sp, 'Class')
	dicoShuf = countLocation.getpG4ShufLocNum(path, sp, 'Class')
...
def getFig3Data(df, path):
	...
	dicoNbTr = countTranscript.getFig3Percent(path, sp)
	Global = pd.DataFrame()
...
def main(path, sp):
	results = path + sp + '/TotDataDensites.csv'
	try:
		...
		# df = removeBiotype(df)
		df = df.reset_index()
		fig3 = getFig3Data(df, path + sp)
		fig3.to_csv(path_or_buf=path+'/Figures/DataFig3.csv', header=True, index=None, sep='\t')
		fig4 = getFigBysubClass(df, path + sp, 'Coding')
		fig4.to_csv(path_or_buf=path+'/Figures/DataFig4.csv', header=True, index=None, sep='\t')
		fig5 = getFigDensityByLocation(df, path + sp, 'Coding')
		fig5.to_csv(path_or_buf=path+'/Figures/DataFig5.csv', header=True, index=None, sep='\t')
		fig6 = getFigBysubClass(df, path + sp, 'LongNC')
		fig6 = fig6.append(getFigBysubClass(df, path + sp, 'ShortNC'))
		fig6.to_csv(path_or_buf=path+'/Figures/DataFig6.csv', header=True, index=None, sep='\t')
		fig7 = getFigDensityByLocation(df, path + sp, 'LongNC')
		fig7.to_csv(path_or_buf=path+'/Figures/DataFig7.csv', header=True, index=None, sep='\t')
		fig8 = getFigDensityByLocation(df, path + sp, 'Pseudogene')
		fig8.to_csv(path_or_buf=path+'/Figures/DataFig8.csv', header=True, index=None, sep='\t')
		supfig1 = getFigBysubClass(df, path + sp, 'Pseudogene')
		supfig1.to_csv(path_or_buf=path+'/Figures/DataSupFig1.csv', header=True, index=None, sep='\t')
