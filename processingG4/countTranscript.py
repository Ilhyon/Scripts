...
def readShufpG4r(pG4rFile, dicoNbTr):
...
def readWtpG4r(filename, dicoNbTr):
	...
	with open(filename) as f:
		...
		for l in lines:
			l = l.rstrip()
			words = l.split('\t')
			if words[10] not in dicoNbTr['Wt']:
				dicoNbTr['Wt'][ words[10] ] = []
			if words[11] not in dicoNbTr['Wt']:
				dicoNbTr['Wt'][ words[11] ] = []
			dicoNbTr['Wt'][ words[10] ].append(words[0])
			dicoNbTr['Wt'][ words[11] ].append(words[0])
	for feature in dicoNbTr['Wt']:
		dicoNbTr['Wt'][feature] = len( set( dicoNbTr['Wt'][feature] ) )
	return dicoNbTr
...
def getFig5Percent(path, sp):
	...
	dfTr = Parser_gtf.importGTFdf(path +'/'+ sp +'.gtf', sp)
	...
	listTrpG4rWt = readWtpG4r(filepG4rWt, dicoNbTr)
...
def getFig3Percent(path, sp):
	filepG4rWt = path + '/pG4.txt'
	filepG4rShuf = path+'Results/All/pG4r_shuffle.csv'
	dfTr = Parser_gtf.importGTFdf(path +'/'+ sp +'.gtf', sp)
	dicoNbTr = { 'Wt' : {}, 'Shuf' : {}, 'Tot' : {} }

	groups = dfTr.groupby('Biotype')
	for name, group in groups:
		if name not in dicoNbTr['Tot']:
			dicoNbTr['Tot'][name] = 0
		dicoNbTr['Tot'][name] += len(set(group.Transcript))
	groups = dfTr.groupby('Type')
	for name, group in groups:
		if name not in dicoNbTr['Tot']:
			dicoNbTr['Tot'][name] = 0
		dicoNbTr['Tot'][name] += len(set(group.Transcript))
	dicoNbTr['Tot']['Global'] = len(set(dfTr.Transcript))
	pprint(dicoNbTr)
