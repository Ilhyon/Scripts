...
def addExonIntron(type, dico, dicoLine):
    ...
    for elem in dicoLine[listName]:
        #Broswe all exons or all introns
        if elem:
...
def findCDS(coord5UTR, coord3UTR, coordExon):
    ...
    if (overlap5UTR == coordExon[1]-coordExon[0] or
    overlap3UTR == coordExon[1]-coordExon[0] or
    overlap5UTR == coordExon[0]-coordExon[1] or
    overlap3UTR == coordExon[0]-coordExon[1]):
        coordCDS = []
...
def getCoordCodon(strand, coord):
    if strand == '1' or strand == '+':
        coordCodon = [coord - 59, coord + 58]
    else:
        coordCodon = [coord + 59, coord - 58]
    return coordCodon
...
def addUTRCDS(dicoLine, dico, dicoBt):
    ...
    if dicoLine['start5'] and dicoLine['end5']:
        ...
        coordStartcodon = getCoordCodon(dicoLine['Strand'], int(dicoLine['end5']))
        idStartcodon = 'StartCodon:'+dicoLine['Chromosome']+':'+str(coordStartcodon[0])+'-'+str(coordStartcodon[1])+':'+dicoLine['Strand']
        if idStartcodon not in dico[ dicoLine['idGene'] ]:
            dico[ dicoLine['idGene'] ][idStartcodon] = {}
        if dicoLine['idTr'] in dicoBt:
            dico[ dicoLine['idGene'] ][idStartcodon].update({dicoLine['idTr'] : dicoBt[ dicoLine['idTr'] ]})
    else:
        coords5UTr = [0, 0]
    if dicoLine['start3'] and dicoLine['end3']:
        ...
        coordStopcodon = getCoordCodon(dicoLine['Strand'], int(dicoLine['start3']))
        idStopcodon = 'StopCodon:'+dicoLine['Chromosome']+':'+str(coordStopcodon[0])+'-'+str(coordStopcodon[1])+':'+dicoLine['Strand']
        if idStopcodon not in dico[ dicoLine['idGene'] ]:
            dico[ dicoLine['idGene'] ][idStopcodon] = {}
        if dicoLine['idTr'] in dicoBt:
            dico[ dicoLine['idGene'] ][idStopcodon].update({dicoLine['idTr'] : dicoBt[ dicoLine['idTr'] ]})
    else:
        coords3UTR = [0, 0]
    if dicoLine['idTr'] in dicoBt:
...
if __name__ == '__main__':
    ...
    outputDir = path+sp+'/'
    dicoInfo = getDicoIndex(path+sp+'/'+sp+'gtf')
