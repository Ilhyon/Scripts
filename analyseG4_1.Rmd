...
SupraFamily[[2]]=NonCoding
SupraFamily[[3]]=Pseudogene
SupraFamily[[4]]=Predictif
```
    ...
    for(i in seq(1,length(NameFile))){# browse all subfamily
      if(is.element(NameFile[i], SupraFamily[[j]])){ # if the subfamily is in the family
        ...
        if(length(SupraFamily[[j]])==16){
          family = 'Coding'
        }else if(length(SupraFamily[[j]])==37){
          family = 'Non Coding'
...
```{r}
MeanGraphic = function(Data, family) {
  ...
  p + geom_bar(stat = "identity",
    ...
    geom_errorbar(limits, position = position_dodge(0.9),
                  width = 0.25) +
    #geom_text(aes(label=NbTRanscriptWithG4), vjust=-0.9,
...
#PercentGraphic(Data_Mouse_perFamily, 'Coding')
```
...
```{r}
Graphic = function(Data, family){
  grid.arrange(PercentGraphic(Data, family), MeanGraphic(Data, family), nrow=2)
}

#Graphic(Data_Mouse_perFamily, 'Coding')
```
